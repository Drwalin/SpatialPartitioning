...
std::vector<std::pair<std::vector<float>, size_t>> broadphasePerformances;
		  ...
		  TestType testType)
{
	...
	std::vector<spp::Aabb> &aabbs = globalAabbs;
	{
		...
		for (; i < testsCount; ++i) {
			ee.push_back((((size_t)mt() % (size_t)MAX_MOVING_ENTITIES) % (TOTAL_ENTITIES-2)) + 2);
	...
	std::vector<SingleTestResult> testsResults;
	testsResults.resize(broadphases.size());
...
// 			printf("\r %6.3f %%     [%6.2f s]", (100.0 * k) / (double)tC, sec);
	...
	for (int i = 0; i < broadphases.size(); ++i) {
		auto &t = broadphasePerformances[i].first;
		...
		broadphasePerformances[i].first[0] = vec.totalTime;
	}
	...
	for (int i = 1; i < argc; ++i) {
		if (std::string(argv[i]).starts_with("-disable-prepass")) {
			...
			printf("Available options:\n"
				   ...
				   "\tTSH_BTDBVT1     - ThreeStageDbvh BvhMedian1 + BulletDbvt\n"
				   "\tTSH_BTDBVT1_NOR - ThreeStageDbvh BvhMedian1 + BulletDbvt (no schedule)\n"
	...
	if (customizeStructures) {
		...
		for (int i = 1; i < argc; ++i) {
			...
			if (false) {
				...
				tsdbvh->SetRebuildSchedulerFunction(EnqueueRebuildThreaded);
				broadphases.push_back(tsdbvh);
			} else if (strcmp(str, "TSH_BTDBVT1") == false) {
				spp::ThreeStageDbvh<spp::Aabb, EntityType, uint32_t, 0> *tsdbvh = new spp::ThreeStageDbvh<spp::Aabb, EntityType, uint32_t, 0>(
				...
				tsdbvh->SetRebuildSchedulerFunction(EnqueueRebuildThreaded);
				broadphases.push_back(tsdbvh);
			} else if (strcmp(str, "TSH_BTDBVT1_NOR") == false) {
				spp::ThreeStageDbvh<spp::Aabb, EntityType, uint32_t, 0> *tsdbvh = new spp::ThreeStageDbvh<spp::Aabb, EntityType, uint32_t, 0>(
					std::make_shared<spp::BvhMedianSplitHeap<spp::Aabb, EntityType, uint32_t, 0, 1>>(TOTAL_ENTITIES),
					nullptr,
					std::make_unique<spp::BulletDbvt<spp::Aabb, EntityType, uint32_t, 0>>());
...
// 			double hours = minutes / 60.0;
	...
	for (int i = 0; i < broadphases.size(); ++i) {
		...
		double usPerOp =
			broadphasePerformances[i].first[0] / broadphasePerformances[i].second;
